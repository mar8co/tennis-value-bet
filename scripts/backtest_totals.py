"""CLI: backtest the over/under total-games market (best-of-3 matches).

    python scripts/backtest_totals.py --tour atp --line 22.5

Run scripts/download_data.py first. The serve/return model is compared
against a naive baseline (the empirical distribution of past totals).
"""
import argparse
import sys
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Backtest the over/under total-games market.")
    ...
    print(f"=== BACKTEST over/under game (best-of-3) - {args.tour.upper()} ===")
    print(f"Periodo        : {_fmt(rec['date'].min())} -> "
          f"{_fmt(rec['date'].max())}")
    print(f"Match valutati : {len(rec)}\n")

    print("ERRORE SULLA MEDIA GAME TOTALI:")
    ...
    print("(Brier/Log loss: piu' basso e' meglio. Baseline = distribuzione "
          "empirica dei totali passati.)\n")
